...
I_STRUCT_NUM = 6;           % struct number
I_PATH_NUM = 7;             % path number
I_NODE_NUM = 8;             % node number
I_LEFTEYE_VALIDITY = 9;     % Left eye validity (0-4)
I_RIGHTEYE_VALIDITY = 10;    % right eye validity (0-4)
I_GAZE_X = 11;              % gaze x
I_GAZE_Y = 12;              % gaze y
I_WAIT_ATTENTION = 13;      % wait for attention in the node
I_OBJ_CENTER_X = 14;        % object center X
I_OBJ_CENTER_Y = 15;        % object center Y
I_OBJ_SIZE = 16;            % object size in pixel
I_KEY_INPUT = 17;           % keyboard input as a number
I_OBSWIN = 18;              % observation windows starting index
...
if version == 4
    ...
    I_OBJ_CENTER_Y = 15;        % object center Y
    I_OBJ_SIZE = 16;            % object size in pixel
    I_OBSWIN = 17;              % observation windows starting index
elseif version == 3
    I_TIME = 1;             % time in ms (Tobii)
    I_TIME_CALL = 2;        % time before call talk2tobii('GET_SAMPLE');
    I_TIME_RESPONSE = 3;    % time after call talk2tobii('GET_SAMPLE');
    ...
    I_GAZE_Y = 12;           % gaze y
    I_WAIT_ATTENTION = 13;  % wait for attention in the node
    I_OBSWIN = 14;           % observation windows starting index    
elseif version == 2
    I_PHASE_NUM = 2;        % phase number
    I_TRIAL_NUM = 3;        % trial number
    I_PATH_NUM = 4;         % path number
    I_STRUCT_NUM = 5;       % struct number
    I_NODE_NUM = 6;         % node number
    I_LEFTEYE_VALIDITY = 7;  % Left eye validity (0-4)
    I_RIGHTEYE_VALIDITY = 8; % right eye validity (0-4)
    I_GAZE_X = 9;           % gaze x
    I_GAZE_Y = 10;           % gaze y
    I_WAIT_ATTENTION = 11;  % wait for attention in the node
    I_OBSWIN = 12;           % observation windows starting index
elseif version == 1
    I_PHASE_NUM = 2;        % phase number
    I_TRIAL_NUM = 3;        % trial number
    I_PATH_NUM = 4;         % path number
...
disp(['Time interval = ' num2str(timeInterval) ' ms']);
